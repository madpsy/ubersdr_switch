# MS-S5 Antenna Switch Firmware (works with UberSDR)
...
- **Custom antenna names** — label each port; names persist across firmware updates
- **OLED display** — shows current antenna, IP address, clock, and lock indicator
- **Configurable startup port** — boot to a fixed default or restore the last-used port
